...
# ⭐ HTML EOB source folder
HTML_FOLDER = r"C:\Renfrew\2.AVATAR\1_HTML-EOB"

# ⭐ Output folder for renamed HTML files
HTML_OUT_FOLDER = r"C:\Renfrew\2.AVATAR\1_HTML-EOB\RENAMED"

# ⭐ DEBUG — announce which script is running
print("\n[DEBUG] Running: system_HTML_rename.py")
print("[DEBUG] Scanning folder:", HTML_FOLDER)
...
def run_era_rename():
    ...
    if not os.path.exists(HTML_FOLDER):
        print("[RENAME] ERROR: HTML EOB folder does not exist:", HTML_FOLDER)
        return
    ...
    for r in proposed_rows:
        ...
        # ⭐ Search inside HTML files for the check number
        for f in html_files:
        ...
        if not match_file:
            print(f"      ✖ No HTML file contains check {chk}")
            continue

        print(f"      ✔ Found HTML file: {match_file}")

        src_path = os.path.join(HTML_FOLDER, match_file)

        # ⭐ KEEP ORIGINAL EXTENSION
        orig_ext = os.path.splitext(match_file)[1]
        dst_path = os.path.join(HTML_OUT_FOLDER, new_name + orig_ext)
